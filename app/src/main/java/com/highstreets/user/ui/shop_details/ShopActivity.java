...
public class ShopActivity extends BaseActivity implements ShopViewInterface, View.OnClickListener, OnMapReadyCallback {
    ...
    public void setShopDetail(final ShopDetail shopDetail) {
        ...
        if (shopDetail.getImages().size() > 0)
            IMAGE_URL = Uri.parse(ApiClient.MERCHANTS_IMAGE_URL + shopDetail.getImages().get(0).getImages());
        ...
        if (shopDetail.getImages().size() > 0) {
            Glide.with(this)
                    ...
                    .load(ApiClient.MERCHANTS_IMAGE_URL + shopDetail.getImages().get(0).getImages())
                    .into(image_one);
        }
        if (shopDetail.getImages().size() > 1) {
            ...
            Glide.with(this)
                    .setDefaultRequestOptions(new RequestOptions().placeholder(R.drawable.place_holder_small))
                    .load(ApiClient.MERCHANTS_IMAGE_URL + shopDetail.getImages().get(1).getImages())
                    .into(image_second);
        }
        if (shopDetail.getImages().size() > 2) {
            Glide.with(this)
                    .setDefaultRequestOptions(new RequestOptions().placeholder(R.drawable.place_holder_small))
                    .load(ApiClient.MERCHANTS_IMAGE_URL + shopDetail.getImages().get(2).getImages())
                    .into(image_third);
        }
        if (shopDetail.getImages().size() == 2) {
